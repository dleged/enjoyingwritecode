(function () {
  ...
  class IIndexDB {
    ...
    constructor({
      ...
      this.store = store;
      const dbPromise = self.openDB(name, version, {
        upgrade(db) {
    ...
    async transaction(transactions) {
      const tx = (await this.getDb()).transaction(this.store, 'readwrite');
      // console.log([
      //   ...transactions.map((v) => {
      ...
      // ]);
      await Promise.all([
        ...transactions.map((v) => tx.store.add(v)),
        tx.done,
      ]);
    }
  ...
  const db = new IIndexDB({
    ...
    version: 1,
    store: 'data_list',
    keyOptions: { keyPath: 'key', autoIncrement: true, },
    schema: ['value', 'string'],
  });

  async function splitStringByUnit(arg) {
    const { data, unit, time, count } = arg
    const remainder = data.length % unit;
    const integer = Math.floor(data.length / unit);
    const splitLength = integer === 0 ? integer : integer + 1;
    const transactions = [];

    for (let i = 0; i < integer; i++) {
      transactions.push({ key: time * count + i, value: data.slice(i * unit, (i + 1) * unit) });
    };

    // 写入 index db
    await db.transaction(transactions);
    self.postMessage({ type: 'runing' });
  }
